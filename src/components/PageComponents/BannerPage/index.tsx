...
const BannerPageContent = ({ data, pagination }: Props) => {
  const bannerPermissions = usePermissions().banner
  ...
  const columns: ColumnDef<TBannerData>[] = useMemo(() => {
    const result: ColumnDef<TBannerData>[] = [
      {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          return (
            <Select value={row.original.status} disabled={!bannerPermissions?.update} onValueChange={() => {
    ...
    if (bannerPermissions?.update || bannerPermissions?.detail || bannerPermissions?.delete) {
      result.push({
        id: 'action',
        header: 'Action',
        cell: ({ row }) => {
    ...
    return result
  }, [bannerPermissions?.update, bannerPermissions?.delete, bannerPermissions?.detail]);
