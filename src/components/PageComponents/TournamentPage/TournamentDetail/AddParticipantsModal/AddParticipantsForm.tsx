...
import { addTournamentParticipant } from '@/lib/api/tournament';
...
const AddParticipantsForm = ({ tournamentCode, availableSlots, joinedUserCodes, onClose }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await addTournamentParticipant({
  ...
  return (
    <div className='flex flex-col gap-6 flex-grow overflow-y-auto'>
      ...
      <ConfirmationModal
        ...
        message={`Add ${selectedMembers.length} participant(s) to this tournament?`}
